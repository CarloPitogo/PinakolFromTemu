...
export function Dashboard() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const [stu, sch, fac, crs] = await Promise.all([
          fetchWithAuth('/students').then(r => r.json()),
          fetchWithAuth('/schedules').then(r => r.json()),
          fetchWithAuth('/faculty').then(r => r.json()),
          fetchWithAuth('/courses').then(r => r.json())
        ]);
    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <section className="bg-gray-900 rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl">
         ...
         <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
            <div>
               <p className="text-[#FF7F11] text-xs font-black uppercase tracking-[0.4em] mb-4">Academic Portal v2.0</p>
            ...
            <div className="flex gap-4">
               <div className="bg-white/5 border border-white/10 p-5 rounded-3xl backdrop-blur-md">
                  ...
                  <p className="text-lg font-black text-white">1st Sem AY 2026-27</p>
               </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-1 space-y-6">
            <Card className="rounded-[2rem] border-none shadow-xl bg-white overflow-hidden">
               ...
               <CardContent className="p-6 space-y-4">
                  ...
                  <Button variant="ghost" className="w-full text-[10px] font-black uppercase tracking-widest text-[#FF7F11] hover:bg-orange-50 mt-2">View All Bulletins</Button>
               </CardContent>
            </Card>

            <Card className="rounded-[2rem] border-none shadow-xl bg-gray-900 overflow-hidden text-white">
               <CardContent className="p-8">
                  <PartyPopper className="w-10 h-10 text-[#FF7F11] mb-4" />
                  <h3 className="text-xl font-black uppercase tracking-tight mb-1 font-sans">Tech Symposium</h3>
                  <p className="text-gray-400 text-[10px] font-black uppercase tracking-widest mb-4">May 12, 2026 • Grand Hall</p>
                  <Button className="w-full bg-[#FF7F11] hover:bg-orange-600 rounded-xl h-10 text-[10px] font-black uppercase tracking-widest transition-transform active:scale-95 shadow-lg shadow-orange-500/20">Register Ticket</Button>
               </CardContent>
            </Card>
         </div>

         {/* Widget: User-Specific Content */}
         <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className={cn("rounded-[2.5rem] p-8 flex flex-col md:flex-row items-center justify-between gap-6 overflow-hidden relative", 
                    ...
                    <div className="flex items-center gap-6 relative z-10">
                       ...
                       <div>
                          ...
                          <p className="text-white/70 text-xs font-black uppercase tracking-widest mt-2">A.Y. 2026-2027 • {currentStudent?.program || 'CCS STUDENT'}</p>
                       </div>
                    </div>
            ...
            {user?.role === 'faculty' && (
               <div className="space-y-6">
                  <div className="rounded-[2.5rem] bg-indigo-600 p-8 text-white flex items-center justify-between shadow-2xl">
                     <div>
